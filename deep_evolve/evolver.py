...
class Evolver():
    ...
    def __init__(self, all_possible_genes, retain=0.15, random_select=0.1,
                 mutate_chance=0.3, opt_for_highest=True):
        """Create an optimizer.
    ...
    def create_population(self, count):
        ...
        assert_msg = (f"Number of population must at least {math.ceil(3 / self.retain)} "
                      f"but is {count}")
        ...
        while i < count:
            ...
            genome = Genome(
                all_possible_genes=self.all_possible_genes,
                geneparam={},
    ...
    def breed(self, mom, dad):
        ...
        genome1 = Genome(self.all_possible_genes, child1,
                         self.ids.get_next_id(), mom.u_id, dad.u_id, self.ids.get_gen())
    ...
    def evolve(self, pop):
        ...
        # Sort on the scores.
        graded = [x[1] for x in sorted(graded, key=lambda x: x[0], reverse=self.opt_for_highest)]
        ...
        # This is wasteful, since we _know_ these are bad, so why keep rescoring
        ...
        # Add children, which are bred from pairs of remaining
        # (i.e. very high or lower scoring) genomes.
